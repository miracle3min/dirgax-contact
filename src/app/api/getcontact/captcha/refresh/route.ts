...
import { callApiRefreshCode, getcontactDecrypt } from '@/lib/getcontact';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call refresh code API
    const apiResult = await callApiRefreshCode(
      credential.token,
      credential.finalKey,
    ...
    if (apiResult.httpCode !== 200 && apiResult.httpCode !== 201) {
      return NextResponse.json(
        {
          message: parsed?.meta?.errorMessage || 'Failed to refresh captcha',
          error: parsed,
        },
